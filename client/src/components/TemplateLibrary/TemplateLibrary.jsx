              ...
              {milestones.map(m=>{
                ...
                return (
                  <div key={`ms-${m.id}`} className="border-b border-gray-100 last:border-0">
                    {/* Milestone row */}
                    <div className="flex items-center px-4 py-2 bg-amber-50/50 group">
                      {/* ↑↓ buttons */}
                      ...
                      <div className="relative shrink-0 mr-1">
                        <div className="text-xs border rounded px-1 py-0.5 max-w-[100px] bg-white hover:border-blue-300 cursor-pointer select-none"
                          onClick={e=>{e.stopPropagation();setOpenResourcePicker(openResourcePicker===m.id?null:m.id);}}>
                          <span className="block truncate min-w-[28px]">{getTaskRoles(m).length>0 ? getTaskRoles(m).join(',') : ''}</span>
                        </div>
                    ...
                    {/* Children */}
                    {children.map(child=>(
                        <div key={`slot-${child.id}`} className="flex items-center px-4 py-2 pl-10 group hover:bg-blue-50/50 border-t border-gray-50">
                          ...
                          <div className="relative shrink-0 mr-1">
                            <div className="text-xs border rounded px-1 py-0.5 max-w-[100px] bg-white hover:border-blue-300 cursor-pointer select-none"
                              ...
                              <span className="block truncate min-w-[28px]">{(()=>{const roles=getTaskRoles(child);return roles.length>0?roles.join(','):'';})()}</span>
                            </div>
                    ...
                    <div key={`slot-${task.id}`} className="flex items-center px-4 py-2 group hover:bg-purple-50/50 border-t border-gray-50">
                      ...
                      <div className="relative shrink-0 mr-1">
                        <div className="text-xs border rounded px-1 py-0.5 max-w-[100px] bg-white hover:border-blue-300 cursor-pointer select-none"
                          ...
                          <span className="block truncate min-w-[28px]">{getTaskRoles(task).length>0 ? getTaskRoles(task).join(',') : ''}</span>
                        </div>
